import { useState } from "react";
import { motion } from "framer-motion";
import {
  Fingerprint,
  DoorOpen,
  Building2,
  CheckCircle,
  ShieldCheck,
  ArrowRight,
  ScanFace,
  LockKeyhole,
  Users,
  Star,
  Wrench,
} from "lucide-react";
import { Link } from "react-router-dom";

import NavbarPage from "../components/NavbarPage";
import Footer from "../components/Footer";
import WhatsAppButton from "../components/WhatsAppButton";
import ScrollTop from "../components/ScrollTop";

const features = [
  "Biometric Fingerprint Systems",
  "RFID & Smart Card Access",
  "Face Recognition Terminals",
  "Employee Attendance Systems",
  "Door Lock Automation",
  "Professional Installation & Support",
];

const steps = [
  "Site Assessment",
  "System Design",
  "Installation",
  "Configuration",
  "Training & Support",
];

const gallery = [
  "/project1.jpg",
  "/project2.jpg",
  "/project3.jpg",
  "/project4.jpg",
];

const partners = [
  "/logos/zkteco.png",
  "/logos/hikvision.png",
  "/logos/dahua.png",
  "/logos/huawei.png",
];

const faqs = [
  {
    q: "Can biometric systems work without internet?",
    a: "Yes. Most biometric devices operate independently and synchronize records whenever a network connection is available.",
  },
  {
    q: "Can I manage multiple doors?",
    a: "Yes. Our systems can control one or multiple doors from a centralized management platform.",
  },
  {
    q: "Do you install attendance systems?",
    a: "Absolutely. We install biometric attendance systems for offices, schools and institutions.",
  },
  {
    q: "Do you provide maintenance?",
    a: "Yes. We offer preventive maintenance, repairs, upgrades and technical support.",
  },
];

export default function AccessControl() {

  const [openFAQ, setOpenFAQ] = useState(null);

  return (
    <>
      <NavbarPage />

      {/* HERO */}

      <section className="pt-36 pb-24 bg-gradient-to-br from-[#031B4E] via-[#062B78] to-[#0A53C1]">

        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">

          <motion.div
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: .8 }}
          >

            <span className="uppercase tracking-[5px] text-cyan-300 font-semibold">
              Access Control Systems
            </span>

            <h1 className="text-5xl lg:text-7xl font-black text-white mt-5 leading-tight">
              Smart Access
              <br />
              Modern Security
            </h1>

            <p className="text-blue-100 text-xl leading-9 mt-8">
              Secure your offices, apartments, schools and businesses
              using biometric fingerprint, facial recognition,
              RFID card and smart access control solutions
              professionally installed by Ogenix Technologies.
            </p>

            <div className="flex flex-wrap gap-4 mt-10">

              <Link
                to="/contact"
                className="bg-cyan-500 hover:bg-cyan-400 text-white px-8 py-4 rounded-xl font-semibold flex items-center gap-2 transition"
              >
                Request Free Quote
                <ArrowRight size={18} />
              </Link>

              <a
                href="tel:+254110405854"
                className="border-2 border-white text-white px-8 py-4 rounded-xl font-semibold hover:bg-white hover:text-[#031B4E] transition"
              >
                Call Now
              </a>

            </div>

          </motion.div>

          <motion.div
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: .8 }}
          >

            <div className="grid grid-cols-2 gap-6">

              <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center">

                <Fingerprint
                  className="mx-auto text-cyan-300"
                  size={60}
                />

                <h3 className="text-white text-xl font-bold mt-5">
                  Fingerprint
                </h3>

              </div>

              <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center">

                <ScanFace
                  className="mx-auto text-cyan-300"
                  size={60}
                />

                <h3 className="text-white text-xl font-bold mt-5">
                  Face Recognition
                </h3>

              </div>

              <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center">

                <LockKeyhole
                  className="mx-auto text-cyan-300"
                  size={60}
                />

                <h3 className="text-white text-xl font-bold mt-5">
                  Smart Locks
                </h3>

              </div>

              <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center">

                <Users
                  className="mx-auto text-cyan-300"
                  size={60}
                />

                <h3 className="text-white text-xl font-bold mt-5">
                  Attendance
                </h3>

              </div>

            </div>

          </motion.div>

        </div>

      </section>

      {/* RESIDENTIAL & COMMERCIAL */}

      <section className="py-24 bg-white">

        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-10">

          <div className="bg-slate-50 rounded-3xl p-10 shadow-lg">

            <DoorOpen
              className="text-blue-600 mb-6"
              size={45}
            />

            <h2 className="text-3xl font-bold mb-5">
              Residential Access Control
            </h2>

            <p className="text-slate-600 leading-8">
              Upgrade your home security with smart locks,
              biometric access and keyless entry systems that
              offer convenience and peace of mind.
            </p>

          </div>

          <div className="bg-slate-50 rounded-3xl p-10 shadow-lg">

            <Building2
              className="text-blue-600 mb-6"
              size={45}
            />

            <h2 className="text-3xl font-bold mb-5">
              Commercial Access Control
            </h2>

            <p className="text-slate-600 leading-8">
              Manage employee and visitor access across offices,
              schools, apartments, hospitals and institutions
              with centralized access management solutions.
            </p>

          </div>

        </div>

      </section>
      {/* FEATURES */}

<section className="py-24 bg-slate-100">

  <div className="max-w-6xl mx-auto px-6">

    <div className="text-center mb-14">

      <span className="uppercase tracking-[5px] text-blue-600 font-semibold">
        Our Solutions
      </span>

      <h2 className="text-4xl lg:text-5xl font-bold mt-4">
        Complete Access Control Services
      </h2>

    </div>

    <div className="grid md:grid-cols-2 gap-6">

      {features.map((feature, index) => (

        <motion.div
          key={index}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: index * 0.08 }}
          className="bg-white rounded-2xl shadow-lg p-6 flex items-center gap-4"
        >

          <CheckCircle
            className="text-green-600"
            size={24}
          />

          <span className="text-lg font-medium">
            {feature}
          </span>

        </motion.div>

      ))}

    </div>

  </div>

</section>

{/* INSTALLATION PROCESS */}

<section className="py-24 bg-slate-50">

  <div className="max-w-6xl mx-auto px-6">

    <div className="text-center mb-14">

      <span className="uppercase tracking-[5px] text-blue-600 font-semibold">
        Our Process
      </span>

      <h2 className="text-4xl font-bold mt-4">
        Professional Installation Process
      </h2>

    </div>

    <div className="grid md:grid-cols-5 gap-8">

      {steps.map((step, index) => (

        <motion.div
          key={index}
          whileHover={{ y: -8 }}
          className="bg-white rounded-3xl shadow-lg p-8 text-center"
        >

          <div className="w-14 h-14 rounded-full bg-blue-600 text-white flex items-center justify-center mx-auto mb-5 text-xl font-bold">

            {index + 1}

          </div>

          <h3 className="font-bold text-lg">

            {step}

          </h3>

        </motion.div>

      ))}

    </div>

  </div>

</section>


{/* WHY CHOOSE US */}

<section className="py-24 bg-gradient-to-r from-[#031B4E] via-[#062B78] to-[#0A53C1]">

  <div className="max-w-7xl mx-auto px-6">

    <div className="text-center mb-16">

      <span className="uppercase tracking-[5px] text-cyan-300 font-semibold">
        Why Choose Ogenix
      </span>

      <h2 className="text-5xl font-bold text-white mt-5">
        Smarter Security Starts Here
      </h2>

      <p className="text-blue-100 max-w-3xl mx-auto mt-6 leading-8">
        Our access control systems combine intelligent technology,
        professional installation and dependable support to keep
        your premises secure while simplifying access management.
      </p>

    </div>

    <div className="grid md:grid-cols-3 gap-8">

      <motion.div
        whileHover={{ y: -8 }}
        className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center border border-white/20"
      >

        <ShieldCheck
          size={48}
          className="mx-auto text-cyan-300 mb-5"
        />

        <h3 className="text-white text-2xl font-bold">
          Maximum Security
        </h3>

        <p className="text-blue-100 mt-4">
          Restrict unauthorized access with reliable authentication
          and smart security solutions.
        </p>

      </motion.div>

      <motion.div
        whileHover={{ y: -8 }}
        className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center border border-white/20"
      >

        <Star
          size={48}
          className="mx-auto text-cyan-300 mb-5"
        />

        <h3 className="text-white text-2xl font-bold">
          Premium Equipment
        </h3>

        <p className="text-blue-100 mt-4">
          We install trusted biometric and access control products
          from globally recognized manufacturers.
        </p>

      </motion.div>

      <motion.div
        whileHover={{ y: -8 }}
        className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 text-center border border-white/20"
      >

        <Wrench
          size={48}
          className="mx-auto text-cyan-300 mb-5"
        />

        <h3 className="text-white text-2xl font-bold">
          Ongoing Support
        </h3>

        <p className="text-blue-100 mt-4">
          Installation, maintenance and technical support whenever
          your business needs assistance.
        </p>

      </motion.div>

    </div>

  </div>

</section>

{/* FAQ */}

<section className="py-24 bg-slate-100">

  <div className="max-w-5xl mx-auto px-6">

    <div className="text-center mb-14">

      <span className="uppercase tracking-[5px] text-blue-600 font-semibold">
        Frequently Asked Questions
      </span>

      <h2 className="text-4xl font-bold mt-4">
        Have Questions?
      </h2>

    </div>

    <div className="space-y-5">

      {faqs.map((faq, index) => (

        <div
          key={index}
          className="bg-white rounded-2xl shadow-lg overflow-hidden"
        >

          <button
            onClick={() =>
              setOpenFAQ(openFAQ === index ? null : index)
            }
            className="w-full flex justify-between items-center p-6 text-left"
          >

            <h3 className="font-bold text-lg">
              {faq.q}
            </h3>

            <span className="text-3xl text-blue-600">
              {openFAQ === index ? "−" : "+"}
            </span>

          </button>

          {openFAQ === index && (

            <div className="px-6 pb-6 text-slate-600 leading-8">
              {faq.a}
            </div>

          )}

        </div>

      ))}

    </div>

  </div>

</section>


{/* RELATED SERVICES */}

<section className="py-24 bg-white">

  <div className="max-w-7xl mx-auto px-6">

    <div className="text-center mb-14">

      <span className="uppercase tracking-[5px] text-blue-600 font-semibold">
        Related Services
      </span>

      <h2 className="text-4xl font-bold mt-4">
        Complete Your Security Solution
      </h2>

    </div>

    <div className="grid md:grid-cols-3 gap-8">

      <Link
        to="/cctv-installation"
        className="bg-slate-50 rounded-3xl p-10 shadow-lg hover:shadow-2xl transition text-center"
      >

        <h3 className="text-2xl font-bold mb-4">
          CCTV Installation
        </h3>

        <p className="text-slate-600">
          Integrate surveillance cameras with your access control system.
        </p>

      </Link>

      <Link
        to="/electric-fence"
        className="bg-slate-50 rounded-3xl p-10 shadow-lg hover:shadow-2xl transition text-center"
      >

        <h3 className="text-2xl font-bold mb-4">
          Electric Fence
        </h3>

        <p className="text-slate-600">
          Add perimeter protection for complete property security.
        </p>

      </Link>

      <Link
        to="/networking"
        className="bg-slate-50 rounded-3xl p-10 shadow-lg hover:shadow-2xl transition text-center"
      >

        <h3 className="text-2xl font-bold mb-4">
          Networking
        </h3>

        <p className="text-slate-600">
          Connect all your smart security devices on one secure network.
        </p>

      </Link>

    </div>

  </div>

</section>


{/* FINAL CTA */}

<section className="py-24 bg-gradient-to-r from-[#031B4E] via-[#062B78] to-[#0A53C1]">

  <div className="max-w-5xl mx-auto px-6 text-center">

    <span className="uppercase tracking-[5px] text-cyan-300 font-semibold">
      Ready to Secure Your Premises?
    </span>

    <h2 className="text-5xl lg:text-6xl font-black text-white mt-5">
      Control Access With Confidence
    </h2>

    <p className="text-blue-100 text-xl leading-9 mt-8 max-w-3xl mx-auto">
      Whether you need fingerprint access, facial recognition,
      smart card systems or complete attendance management,
      Ogenix Technologies has the right solution for you.
    </p>

    <div className="flex flex-col sm:flex-row justify-center gap-5 mt-12">

      <Link
        to="/contact"
        className="bg-cyan-500 hover:bg-cyan-400 text-white px-10 py-4 rounded-xl font-semibold transition"
      >
        Request Free Quote
      </Link>

      <a
        href="tel:+254110405854"
        className="border-2 border-white text-white px-10 py-4 rounded-xl font-semibold hover:bg-white hover:text-[#031B4E] transition"
      >
        Call +254 110 405 854
      </a>

    </div>

  </div>

</section>

<Footer />
<WhatsAppButton />
<ScrollTop />

    </>
  );
}